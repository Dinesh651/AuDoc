...
const ChecklistItem: React.FC<{
  ...
  onEdit?: () => void;
  disabled?: boolean;
}> = ({ id, label, checked, onChange, onEdit, disabled }) => (
  <div className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-md transition-colors group w-full">
    <div className="relative flex items-start flex-grow">
        <div className="flex items-center h-5">
            ...
            onChange={onChange}
            disabled={disabled}
            className={`h-4 w-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
            />
        </div>
        <div className="ml-3 text-sm">
            <label htmlFor={id} className={`select-none font-medium ${checked ? 'text-slate-500 line-through decoration-slate-400' : 'text-slate-700 group-hover:text-indigo-700'} ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
            {label}
            </label>
        </div>
    </div>
...
// --- Work Paper Modal ---
// (Unchanged logic, just ensure onSave checks permission in parent or disabled inputs)
...
const WorkPaperModal: React.FC<{
    ...
    onClose: () => void;
    isReadOnly?: boolean;
}> = ({ isOpen, title, type, data, onSave, onClose, isReadOnly }) => {
    ...
    const inputBg = isReadOnly ? 'bg-slate-100 cursor-not-allowed' : 'bg-slate-50';
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Working Paper Environment {isReadOnly && <span className="text-amber-600 font-bold ml-2">(Read Only)</span>}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                                ...
                                <table className="min-w-full divide-y divide-slate-200">
                                    <thead className="bg-slate-50">
                                        <tr>
                                            ...
                                            {!isReadOnly && <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase">Action</th>}
                                        </tr>
                                    </thead>
                    ...
                    {/* ... (Other types follow similar read-only pattern, omitted for brevity but logic applies via isReadOnly prop passed in) ... */}
                    {/* Fallback for Text Types */}
                    {['reperformance', 'observation', 'inquiry', 'analyticalProcedures', 'externalConfirmation', 'recalculation'].includes(type) && (
                         <div className="h-full flex flex-col">
                             {/* Render recalculation differently if needed, but for simplicity handling text here */}
                             {type === 'recalculation' ? (
                                 <div className="space-y-6">
                                     {!isReadOnly && (
                                         <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                                            ...
                                            <tbody className="divide-y divide-slate-200">
                                                {Array.isArray(localData) && localData.map((item: RecalculationItem) => (
                                                    <tr key={item.id} className="hover:bg-slate-50">
                                                        ...
                                                        {!isReadOnly && <td className="px-4 py-3 text-right"><button onClick={() => removeCalculation(item.id)} className="text-red-500">&times;</button></td>}
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                ...
                                <div className="h-full flex flex-col">
                                    ...
                                    <textarea 
                                        className={`flex-1 w-full p-4 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-inner resize-none text-slate-700 leading-relaxed ${isReadOnly ? 'bg-slate-100 cursor-not-allowed' : 'bg-white'}`}
...
const AuditEvidence: React.FC<AuditTabProps> = ({ client, engagementId, isReadOnly }) => {
  const [expandedSection, setExpandedSection] = useState<string | null>('sa500');
  const [isLoaded, setIsLoaded] = useState(false);

  // Consolidated State (Same as before)
  ...
  const handleSaveWorkPaper = (newData: any) => {
      if (!activeWorkPaper || isReadOnly) return;
  ...
  const addConfirmation = () => {
    if (newConfirm.partyName && newConfirm.sentDate && !isReadOnly) {
  ...
  const addRelatedParty = () => {
    if (newParty.name && newParty.relationship && !isReadOnly) {
  ...
  const inputClass = isReadOnly ? 'bg-slate-100 cursor-not-allowed' : 'bg-white';
  const inputClass50 = isReadOnly ? 'bg-slate-100 cursor-not-allowed' : 'bg-slate-50';
        ...
        {expandedSection === 'sa500' && (
          <div className="p-6 border-t border-slate-100 bg-slate-50/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              <ChecklistItem id="insp" label="Inspection of Records/Docs" checked={data.sa500.checklist.inspection} onChange={() => toggleChecklist('sa500', 'checklist', 'inspection')} onEdit={() => setActiveWorkPaper({ type: 'inspection', title: 'Inspection of Records or Documents' })} disabled={isReadOnly} />
              {/* ... Other checklist items with disabled={isReadOnly} ... */}
              <ChecklistItem id="obs" label="Observation of Processes" checked={data.sa500.checklist.observation} onChange={() => toggleChecklist('sa500', 'checklist', 'observation')} onEdit={() => setActiveWorkPaper({ type: 'observation', title: 'Observation' })} disabled={isReadOnly}/>
              <ChecklistItem id="ext" label="External Confirmation" checked={data.sa500.checklist.externalConfirmation} onChange={() => toggleChecklist('sa500', 'checklist', 'externalConfirmation')} onEdit={() => setActiveWorkPaper({ type: 'externalConfirmation', title: 'External Confirmation' })} disabled={isReadOnly}/>
              <ChecklistItem id="recal" label="Recalculation" checked={data.sa500.checklist.recalculation} onChange={() => toggleChecklist('sa500', 'checklist', 'recalculation')} onEdit={() => setActiveWorkPaper({ type: 'recalculation', title: 'Recalculation' })} disabled={isReadOnly}/>
              <ChecklistItem id="reper" label="Reperformance" checked={data.sa500.checklist.reperformance} onChange={() => toggleChecklist('sa500', 'checklist', 'reperformance')} onEdit={() => setActiveWorkPaper({ type: 'reperformance', title: 'Reperformance' })} disabled={isReadOnly}/>
              <ChecklistItem id="anal" label="Analytical Procedures" checked={data.sa500.checklist.analyticalProcedures} onChange={() => toggleChecklist('sa500', 'checklist', 'analyticalProcedures')} onEdit={() => setActiveWorkPaper({ type: 'analyticalProcedures', title: 'Analytical Procedures' })} disabled={isReadOnly}/>
              <ChecklistItem id="inq" label="Inquiry" checked={data.sa500.checklist.inquiry} onChange={() => toggleChecklist('sa500', 'checklist', 'inquiry')} onEdit={() => setActiveWorkPaper({ type: 'inquiry', title: 'Inquiry' })} disabled={isReadOnly}/>
            </div>
            <div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-slate-100 hover:shadow-md transition-shadow">
        ...
        {expandedSection === 'sa501' && (
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 space-y-6">
            <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
                ...
                <div className="flex flex-wrap items-center gap-6 mb-4">
                    <label className="flex items-center cursor-pointer p-2 hover:bg-slate-50 rounded-md transition-colors">
                        <input type="checkbox" checked={data.sa501.inventory.attendedCount} onChange={() => updateNestedField('sa501', 'inventory', 'attendedCount', !data.sa501.inventory.attendedCount)} disabled={isReadOnly} className="h-5 w-5 text-indigo-600 rounded focus:ring-indigo-500" />
                        <span className="ml-3 text-sm font-medium text-slate-700">Attended Physical Count</span>
                    </label>
                    <div className="flex-1 min-w-[200px]">
                        <input type="date" value={data.sa501.inventory.countDate} onChange={(e) => updateNestedField('sa501', 'inventory', 'countDate', e.target.value)} disabled={isReadOnly} className={`w-full p-2.5 border border-slate-200 rounded-md text-sm outline-none ${inputClass50}`} />
                    </div>
                </div>
                <textarea 
            ...
            {/* ... Litigation and Segment Information similarly updated with inputClass and disabled={isReadOnly} ... */}
             <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-4 pb-2 border-b border-slate-100">
                    <h4 className="font-bold text-slate-800">Litigation & Claims</h4>
                </div>
                <div className="flex flex-wrap gap-6 mb-4">
                    <ChecklistItem id="lit_inq" label="Inquiry of Management/Legal" checked={data.sa501.litigation.inquiryManagement} onChange={() => updateNestedField('sa501', 'litigation', 'inquiryManagement', !data.sa501.litigation.inquiryManagement)} disabled={isReadOnly} />
                    <ChecklistItem id="lit_rev" label="Reviewed Legal Expenses" checked={data.sa501.litigation.reviewedLegalExpenses} onChange={() => updateNestedField('sa501', 'litigation', 'reviewedLegalExpenses', !data.sa501.litigation.reviewedLegalExpenses)} disabled={isReadOnly} />
                </div>
                <textarea 
                    className={`w-full p-3 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-indigo-500 ${inputClass}`}
                    rows={2}
                ...
                <table className="min-w-full divide-y divide-slate-300 bg-white">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            {!isReadOnly && <th className="px-3 py-3.5 text-right text-xs font-semibold text-slate-900">Action</th>}
                        </tr>
                    </thead>
                            ...
                            <tr key={req.id}>
                                ...
                                <td className="whitespace-nowrap px-3 py-4">
                                    <select 
                                        ...
                                        disabled={isReadOnly}
                                        className={`text-xs font-bold px-2 py-1 rounded-full border-0 focus:ring-2 focus:ring-indigo-500 ${isReadOnly ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'} bg-slate-100`}
                                    >
